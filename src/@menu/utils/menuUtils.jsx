...
//   return false
// }
export const confirmUrlInChildren = (children, pathname) => {
  return Children.toArray(children).some(child => {
    if (!isValidElement(child)) return false

    const props = child.props
    const activeUrl = props.activeUrl || props.href
    const urls = Array.isArray(activeUrl) ? activeUrl : [activeUrl]

    // Check direct match
    const isDirectMatch = urls.some(url =>
      typeof url === 'string' && (pathname === url || pathname.startsWith(url))
    )
    ...
    if (props.children) {
      return confirmUrlInChildren(props.children, pathname)
    }
