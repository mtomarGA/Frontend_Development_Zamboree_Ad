...
import getAllBrand from '@/services/product/product-brand'
...
const ProductBrandTable = () => {
    ...
    const getProductBrands = async () => {
        const res = await getAllBrand.getProductBrand()
        setData(res.data)
    }

    const handleDelete = async (id) => {
        const result = await getAllBrand.deleteProductBrand(id)
    ...
    const columns = useMemo(() => {
        ...
        return [
            ...
            columnHelper.accessor('logo', {
                ...
                cell: ({ row }) => (
                    <img
                        src={row.original?.image} // ✅ make sure this matches your data key
                        alt="logo"
        ...
        <Card className='shadow-none '>
            ...
            <Dialog fullWidth open={modalOpen} maxWidth='sm' scroll='body' sx={{ '& .MuiDialog-paper': { overflow: 'visible' } }}>
                ...
                <DialogTitle variant='h4' className='flex gap-2 flex-col text-center '>
                    <ProductBrandDialoag onsuccess={setModalOpen} getProductBrands={getProductBrands} EditSelectedPost={EditSelectedPost} />
                </DialogTitle>
            </Dialog>
        </Card>
    )
