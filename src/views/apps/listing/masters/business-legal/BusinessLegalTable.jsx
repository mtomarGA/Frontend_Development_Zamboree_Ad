...
import AddBusinessLegal from '@components/dialogs/add-businessLegal'
...
const BusinessLegalTable = () => {
  ...
  const handleEditNature = data => {
    setEditData(data)
    setOpen(true)
  }

  const handleGetStatus = async () => {
    const res = await businessStatusService.getAllBusinessStatuss()
    console.log(res.data, 'data data')
    setData(res.data)
  }

  const handleDeleteNature = async id => {
    const res = await businessStatusService.deleteBusinessStatus(id)
    if (res.statusCode == 200) {
      toast.success('Business Status Deleted')
      handleGetStatus()
    } else {
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            {hasPermission("partner_master:edit") &&
              <IconButton onClick={() => handleEditNature(row.original)}>
                <i className='tabler-edit text-blue-500' />
              </IconButton>}
            {hasPermission("partner_master:delete") &&
              <DeleteConfirmationDialog
                itemName="Legal status"
                onConfirm={() => handleDeleteNature(row.original._id)}
  ...
  return (
    <>
        ...
        <div className='flex flex-wrap justify-between gap-4 p-6'>
          ...
          <div className='flex max-sm:flex-col items-start sm:items-center gap-4 max-sm:is-full'>
            ...
            {hasPermission("partner_master:add") &&
              ...
              >
                Add Business Legal Status
              </Button>
            }
            {/* <OpenDialogOnElementClick element={Button} elementProps={buttonProps} dialog={AddBusinessLegal} /> */}
          </div>
        </div>
      ...
      <AddBusinessLegal open={open} setOpen={setOpen} data={editData} onSuccess={handleGetStatus} />
    </>
  )
}

export default BusinessLegalTable
