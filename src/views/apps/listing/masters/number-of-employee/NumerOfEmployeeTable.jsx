...
import AddNumberOfEmployee from '@components/dialogs/add-numberEmployee'
...
const NumerOfEmployeeTable = () => {
  ...
  const handleGetEmployee = async () => {
    const res = await employeeNoService.getAllEmpoyeeNos()
    console.log(res.data, 'res res')
    setData(res.data)
  }

  const handleDeleteEmployee = async id => {
    const res = await employeeNoService.deleteEmpoyeeNo(id)
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            {hasPermission("partner_master:edit") &&
              <IconButton onClick={() => handleEditEmployee(row.original)}>
                <i className='tabler-edit text-blue-500' />
              </IconButton>}
            {hasPermission("partner_master:delete") &&
              <DeleteConfirmationDialog
                itemName="number of employee"
                onConfirm={() => handleDeleteEmployee(row.original._id)}
  ...
  return (
    <>
      ...
      <AddNumberOfEmployee open={open} setOpen={setOpen} data={editData} onSuccess={handleGetEmployee} />
    </>
  )
}

export default NumerOfEmployeeTable
