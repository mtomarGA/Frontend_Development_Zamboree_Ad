...
import AddDocumentType from '@components/dialogs/add-documentType'
...
const DocumentTypeTable = () => {
  ...
  const handleGetKyc = async () => {
    const res = await kycTypeService.getAllKycTypes()
    setData(res.data)
  }

  const handleDeleteDocType = async id => {
    const res = await kycTypeService.deleteKycType(id)
          ...
          <div className='flex items-center'>
            ...
            {hasPermission("partner_master:edit") &&
              <DeleteConfirmationDialog
                itemName="document type"
                onConfirm={() => handleDeleteDocType(row.original._id)}
  ...
  return (
    <>
      <Card>
        <div className='flex flex-wrap justify-between gap-4 p-6'>
          ...
          <div className='flex max-sm:flex-col items-start sm:items-center gap-4 max-sm:is-full'>
            ...
            {/* <OpenDialogOnElementClick element={Button} elementProps={buttonProps} dialog={AddDocumentType} /> */}
          </div>
        </div>
      ...
      <AddDocumentType open={open} setOpen={setOpen} data={editData} onSuccess={handleGetKyc} />
    </>
  )
}
