...
const ViewCategoryTable = () => {
  ...
  const handleEditCategory = id => {
    router.push(`/en/apps/listing/masters/update-category/${id}`)
  }
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('iconImage', {
        header: 'Icon',
        cell: ({ row }) => (
          <Typography>
            <img
              src={`${row.original.iconImage}`}
              width={38}
              height={38}
      ...
      columnHelper.accessor('thumbImage', {
        header: 'Thumbnail',
        cell: ({ row }) => (
          <Typography>
            <img
      ...
      columnHelper.accessor('bannerImageApp', {
        header: 'App Banner',
        cell: ({ row }) => (
          <Typography>
            <img
              src={`${row.original.bannerImageApp}`}
              width={38}
              height={38}
              className='rounded bg-actionHover'
            />
      ...
      columnHelper.accessor('bannerImageWeb', {
        header: 'Web Banner',
        cell: ({ row }) => (
          <Typography>
            <img
              src={`${row.original.bannerImageWeb}`}
              width={38}
              height={38}
              className='rounded bg-actionHover'
            />
      ...
      columnHelper.accessor('totalListing', {
        header: 'Total Listing',
        cell: ({ row }) => <Typography>{'22'}</Typography>
      }),
      ...
      columnHelper.accessor('actions', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            <IconButton onClick={() => handleEditCategory(row.original._id)}>
        ...
        <div className='flex flex-wrap justify-between gap-4 p-6'>
          ...
          <div className='flex max-sm:flex-col items-start sm:items-center gap-4 max-sm:is-full'>
            ...
            <Button
              variant='contained'
              className='max-sm:is-full'
              onClick={() => router.push('/en/apps/listing/masters/add-category')}
              // href={getLocalizedUrl('/apps/listing/masters/add-category', locale)}
