...
import AddStates from '@components/dialogs/add-states'
import OpenDialogOnElementClick from '@components/dialogs/OpenDialogOnElementClick'
import AddStatesInfo from '@components/dialogs/add-states'
...
const StateListTable = ({ productData }) => {
  useEffect(() => {
  ...
  const handleDelete = async id => {
    const res = await stateService.deleteState(id)
    toast.success('State Deleted Successfully')
    handleGet()
  }

  const handleGet = async () => {
    const res = await stateService.allStateGet()
    setData(res.data)
    setFilteredData(res.data)
  }

  const handleEditState = async (id) => {
    const stateData = await stateService.getSingleState(id)
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            {hasPermission("partner_master:edit") &&
              <IconButton onClick={() => handleEditState(row.original._id)}>
  ...
  const table = useReactTable({
    ...
    // Limit global filtering to the 'name' column only
  ...
  return (
    <>
      ...
      <AddStatesInfo open={open} setOpen={setOpen} data={editData} onSuccess={handleGet} />
    </>
  )
}
