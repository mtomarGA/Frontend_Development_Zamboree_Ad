...
import AddBusinessNature from '@components/dialogs/add-businessNature'
...
const BusinessNatureTable = () => {
  ...
  const handleGetNature = async () => {
    const res = await businessNatureService.getAllBusinessNatures()
    setData(res.data)
  }

  const handleDeleteNature = async id => {
    const res = await businessNatureService.deleteBusinessNature(id)
    if (res.statusCode == 200) {
      toast.success('Business Nature Deleted')
      handleGetNature()
    } else {
  ...
  return (
    <>
      ...
      <AddBusinessNature open={open} setOpen={setOpen} data={editData} onSuccess={handleGetNature} />
    </>
  )
}

export default BusinessNatureTable
