...
const AttributeListTable = () => {
    ...
    const handleGet = async () => {
        const res = await serviceAttributeService.getServiceAttribute()
        setData(res.data)
    ...
    const handleAdd = async () => {
        ...
        try {
            await serviceAttributeService.addServiceAttribute({
    ...
    const handleUpdate = async () => {
        ...
        try {
            await serviceAttributeService.updateServiceAttribute(editId, {
                name: newAttribute.trim(),
                status
            })
    ...
    const handleDelete = async id => {
        await serviceAttributeService.deleteServiceAttribute(id)
    ...
    const columns = useMemo(() => [
        ...
        columnHelper.accessor('categories.name', {
            ...
            cell: ({ row }) => <Typography>{row.original?.categories?.name}</Typography>
        }),
                    ...
                    <div className='flex items-center gap-2'>
                        <>
                            {hasPermission("partner_master:edit") && (
                                <IconButton onClick={() => router.push(`/en/apps/listing/service/attribute/update/${row.original._id}`)}>
            ...
            <div className='flex flex-wrap justify-between items-center gap-4 p-6'>
                ...
                {hasPermission("partner_master:add") &&
                    <div className='ml-auto'>
                        <Button variant='contained' onClick={() => router.push('/en/apps/listing/service/attribute/create')}>
                            Add Attribute
                        </Button>
                    </div>}
            </div>
