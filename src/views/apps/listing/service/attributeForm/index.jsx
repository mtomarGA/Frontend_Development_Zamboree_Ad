...
const AttributeForm = ({ mode, id = null }) => {
    ...
    useEffect(() => {
        if (mode === 'edit' && id) {
            ...
            serviceAttributeService.getServiceSingleAttribute(id).then(res => {
                const data = res.data
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (mode === 'create') {
                const res = await serviceAttributeService.addServiceAttribute(payload);
                ...
                await serviceAttributeService.updateServiceAttribute(id, payload);
                toast.success('Attribute updated');
            }
            router.push('/en/apps/listing/service/attribute');
        } catch (err) {
            toast.error(err?.response?.data?.message || ' attribute');
        }
    }

    const handleAddValue = () => {
        setValues([...values, { id: Date.now().toString(), text: '' }])
    }
            ...
            {tab === 0 && (
                <div className="space-y-4">
                    ...
                    <FormControlLabel
                        ...
                        label="Use this attribute for filtering"
                    />
                    ...
                    <Button className='flex' variant="contained" onClick={() => setTab(1)}>Next</Button>
                </div>
            )}
