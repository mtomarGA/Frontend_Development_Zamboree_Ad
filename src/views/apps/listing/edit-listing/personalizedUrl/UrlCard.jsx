...
const PersonalizedUrlForm = ({ nextHandle }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!isVAlid) return
    console.log('url Data:', urlFormData)
    // nextHandle()
  }
  ...
  const handleCheckAvailability = async (suggestionCheck = false, suggestionURL = "") => {
    ...
    if (urlToCheck && !isChecking) {
      try {
        setIsChecking(true)
        // Clear previous suggestions when checking new URL
        setSuggestions([])
      ...
      <CardContent component="form" onSubmit={handleSubmit}>
        ...
        <Box display='flex' gap={2} alignItems='center' mb={2}>
          ...
          <Button
            ...
            disabled={!urlFormData.url?.trim() || isChecking}
          >
        ...
        {urlAvailable && (
          <Alert severity='success' sx={{ mb: 3 }}>
            🎉 Congratulations! <strong>https://www.happiningads.com/{url}</strong> is available.
            <br />
            Click on <strong>"Save"</strong> to make this FREE Catalog URL.
          </Alert>
        )}
        ...
        {suggestions.length > 0 && (
          <Box sx={{ mb: 3 }}>
            ...
            <Typography variant='caption' sx={{ mt: 1, display: 'block', color: 'text.secondary' }}>
              Click on any suggestion to select it and check availability
            </Typography>
          </Box>
        )}

        {!urlAvailable && !showWarning && urlFormData.url?.trim() && !isChecking && suggestions.length === 0 && (
          <Typography variant='body2' sx={{ mb: 2, color: 'text.secondary' }}>
            Enter a URL and click "Check Availability" to see suggestions if it's taken.
          </Typography>
        )}
