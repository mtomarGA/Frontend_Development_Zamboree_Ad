...
function ArtistTable({ handleClickOpen, GetCategoryFun, quizType }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await ArtistService.putData(EditData._id, EditData)
    ...
    const deleteid = async (id) => {
        try {
            const res = await ArtistService.deleteData(id)
        ...
        <div>
                        ...
                        <div>
                            ...
                            {EditData.image && (
                                <Typography variant='body2' className='mt-2 text-green-700'>
                                    Selected: {imageName} <Avatar src={EditData?.image} alt='' className='w-10 h-10' />
                                </Typography>
                            )}
                            {formErrors.image && (
            ...
            <TableContainer className='shadow p-6'>
                <div className='flex justify-between items-center mb-4'>
                    <h3 className='mb-4'>Event Artist</h3>
                        ...
                        <TableBody>
                            {paginatedData.map((row, index) => (
                                ...
                                <TableRow key={row._id}>
                                    ...
                                    <TableCell className='p-2'>{row?.profession}</TableCell>
                                    <TableCell className='p-2'>
                                        <Chip label={row.status} color={statusStyles[row?.status]} variant='tonal' />
                                    </TableCell>
                                    <TableCell className='p-2'>
                                        {/* {hasPermission('event_masters:view') &&
                                        <Eye
                                            onClick={() => Editid(row)}
