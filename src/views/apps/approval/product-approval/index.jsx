...
import ViewApprovalDetail from '@/components/dialogs/view-product/index'
// Import Services
...
const ViewProductTable = () => {
    ...
    const handleGetProduct = async () => {
        const res = await approvalService.getPendingProduct()
        console.log(res?.data, "data data data");

        setData(res?.data)
    }

    const handleViewProduct = (product, images) => {
        setShowData({ product, images })
        setOpen(true)
    }
        ...
        () => [
            ...
            columnHelper.accessor('status', {
                ...
                cell: ({ row }) => {
                    ...
                    const hasPendingImage = product.images?.some(img => img.status === 'PENDING');
                    const isPending = product.status === 'PENDING' || hasPendingImage;
                ...
                cell: ({ row }) => (
                    <div className='flex items-center'>
                        {hasPermission("product_manage_product_group:edit") &&
                            <IconButton >
                                <i className='tabler-eye text-blue-500' onClick={() => handleViewProduct(row.original, row.original?.images)} />
                            </IconButton>}
    ...
    return (
        <>
            ...
            <ViewApprovalDetail open={open} setOpen={setOpen} data={showData} onSuccess={handleGetProduct} />
        </>
    )
}
