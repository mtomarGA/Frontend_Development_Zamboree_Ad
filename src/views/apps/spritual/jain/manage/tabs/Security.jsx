...
const Security = ({ nextHandle, handleCancel }) => {
  ...
  const handleSendOTP = async (type) => {
    if (type === 'email') {
      await JainService.sendEmailOTP({ email: securityFormData.email, name: "Gahram User" })
    } else {
      await JainService.sendMobileOTP(securityFormData.phoneNumber)
    }
  ...
  const verifyOTP = async (otp, type) => {
    ...
    if (type === 'email') {
      const res = await JainService.verifyEmailOTP({ email: securityFormData.email, otp })
      ...
      const res = await JainService.verifyMobileOTP({ mobile: securityFormData.phoneNumber, otp })
