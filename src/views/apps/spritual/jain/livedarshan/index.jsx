...
import JainLiveDarshanService from '@/services/spritual/jainLiveDarshan'
...
const LiveDarshan = ({ handleAddEmployee }) => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const response = await JainLiveDarshanService.getAll()
  ...
  const handleAdd = async (values) => {
    try {
      setLoading(true)
      await JainLiveDarshanService.create(values)
      fetchData()
      setOpen(false)
    } catch (error) {
  ...
  const handleUpdate = async (values) => {
    try {
      setLoading(true)
      await JainLiveDarshanService.update(editValue._id, values)
      fetchData()
      setOpen(false)
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      setLoading(true)
      await JainLiveDarshanService.delete(id)
      fetchData()
    } catch (error) {
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex gap-2'>
            ...
            {hasPermission('spiritual_jain_live_darshan:edit') && <button onClick={() => {
            ...
            ></button>}
            {hasPermission('spiritual_jain_live_darshan:delete') && <button onClick={() => handleDelete(row.original._id)}
    ...
    <Grid className='' container>
      ...
      <Grid className='w-full' item xs={12}>
          ...
          <Card>
            <CardContent className='flex flex-col gap-4 sm:flex-row items-start sm:items-center justify-between flex-wrap'>
              ...
              <div className='flex flex-wrap gap-4'>
                ...
                />
                {hasPermission('spiritual_jain_live_darshan:add') && <Button variant='contained' onClick={() => {setOpen(true); setEditValue(null)}}>
                  Add Live Darshan
                </Button>}
              </div>
            </CardContent>
        ...
        <LiveDarshanModal
          ...
          handleAdd={handleAdd}
          folderName='Spiritual/Jainism/LiveDarshan'
