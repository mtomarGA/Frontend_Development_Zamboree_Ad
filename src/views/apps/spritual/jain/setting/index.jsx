...
import JainSettingService from '@/services/spritual/jainSetting'
...
const JainSetting = () => {
    ...
    const [settings, setSettings] = useState({
        ...
        featured: {
            ...
            pooja: { ...defaultImage },
            chalisa: { ...defaultImage },
            paath: { ...defaultImage },
            bhajan: { ...defaultImage },
            parv_katha: { ...defaultImage },
            jain_kahaniya: { ...defaultImage },
            stotra: { ...defaultImage },
        },
        ad_slider: [],
        // mandir_page: [{ ...defaultImage }],
        // aarti_chalisa: [{ ...defaultImage }],
        // dharmik_gyaan: [{ ...defaultImage }],
        // festival: [{ ...defaultImage }],
        // articles: [{ ...defaultImage }],
        // live_darshan: [{ ...defaultImage }],
        // mandir_page", "festival", "paath", "pooja" , "chalisa" , "aarti",  , "bhajan", "parv_katha" , "jain_kahaniya", "stotra
        mandir: [{ ...defaultImage }],
        festival: [{ ...defaultImage }],
        paath: [{ ...defaultImage }],
        pooja: [{ ...defaultImage }],
    ...
    const fetchSettings = async () => {
        try {
            const response = await JainSettingService.get()
            ...
            if (response && response.data) {
                ...
                const mergedSettings = {
                    ...
                    jainism_page: ensureArray(rest.jainism_page, defaultImage),
                    chants: ensureArray(rest.chants, defaultChant),
                    featured: {
                        mandir: { ...defaultImage, ...rest.featured?.mandir },
                        aarti: { ...defaultImage, ...rest.featured?.aarti },
                        live_darshan: { ...defaultImage, ...rest.featured?.live_darshan },
                        festival: { ...defaultImage, ...rest.featured?.festival },
                        pooja: { ...defaultImage, ...rest.featured?.pooja },
                        chalisa: { ...defaultImage, ...rest.featured?.chalisa },
                        paath: { ...defaultImage, ...rest.featured?.paath },
                        bhajan: { ...defaultImage, ...rest.featured?.bhajan },
                        parv_katha: { ...defaultImage, ...rest.featured?.parv_katha },
                        jain_kahaniya: { ...defaultImage, ...rest.featured?.jain_kahaniya },
                        stotra: { ...defaultImage, ...rest.featured?.stotra },
                    },
                    ad_slider: ensureArray(rest.ad_slider, { image: '', businessId: '' }),
                    mandir: ensureArray(rest.mandir, defaultImage),
                    festival: ensureArray(rest.festival, defaultImage),
                    paath: ensureArray(rest.paath, defaultImage),
                    pooja: ensureArray(rest.pooja, defaultImage),
                    chalisa: ensureArray(rest.chalisa, defaultImage),
                    aarti: ensureArray(rest.aarti, defaultImage),
                    bhajan: ensureArray(rest.bhajan, defaultImage),
                    parv_katha: ensureArray(rest.parv_katha, defaultImage),
                    jain_kahaniya: ensureArray(rest.jain_kahaniya, defaultImage),
                    stotra: ensureArray(rest.stotra, defaultImage),
    ...
    const onImageSelect = async (file, cropContext) => {
        ...
        } else if (['mandir', 'aarti_chalisa', 'dharmik_gyaan', 'festival', 'articles', 'live_darshan'].includes(cropContext.section)) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await JainSettingService.update(settings)
    ...
    return (
        <Grid container spacing={2}>
            <Grid item xs={12}>
                <Typography variant="h4">Jain Settings</Typography>
            </Grid>
            ...
            <Grid item xs={12}>
                <Card className="p-2">
                    <Grid container justifyContent={'space-between'} alignItems={'center'} className="mb-1">
                        <Typography variant="body1" className="font-semibold">Jainism Page</Typography>
                        <Button
                            variant="contained"
                            size="small"
                            onClick={() => {
                                setSettings(prev => ({
                                    ...prev,
                                    jainism_page: [...prev.jainism_page, { ...defaultImage }]
                                }))
                            }}
                        >
                            Add Banner
                    ...
                    <Grid container spacing={1}>
                        {settings.jainism_page.map((banner, bannerIndex) => (
                                ...
                                <Grid container spacing={1} alignItems="center">
                                    <Grid item xs={12}>
                                        <Grid container justifyContent={'space-between'} alignItems={'center'}>
                                            ...
                                            {settings.jainism_page.length > 1 && (
                                                <IconButton
                                                    size="small"
                                                    color="error"
                                                    onClick={() => {
                                                        const updated = [...settings.jainism_page]
                                                        updated.splice(bannerIndex, 1)
                                                        setSettings(prev => ({ ...prev, jainism_page: updated }))
                                                    }}
                                                >
                                    ...
                                    {['web_image', 'mobile_image'].map(type => {
                                        const fieldKey = `jainism_page_${bannerIndex}_${type}`;
                                        return (
            ...
            {/* Additional Pages */}
            {["mandir", "festival", "paath", "pooja" , "chalisa" , "aarti", "bhajan", "parv_katha" , "jain_kahaniya", "stotra", "live_darshan"].map((section) => (
            ...
            {/* Submit Button */}
            <Grid item xs={12}>
                {hasPermission('spiritual_jain_setting:add') && <Button
                    variant="contained"
                    color="primary"
                    fullWidth
