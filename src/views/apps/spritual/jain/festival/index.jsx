...
const JainFestival = ({ handleAddEmployee }) => {
  // States
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const response = await JainFestivalService.getAll()
  ...
  const handleAdd = async (values) => {
    try {
      setLoading(true)
      await JainFestivalService.create(values)
      fetchData()
      setOpen(false)
    } catch (error) {
  ...
  const handleUpdate = async (values) => {
    try {
      setLoading(true)
      await JainFestivalService.update(editValue._id, values)
      fetchData()
      setOpen(false)
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      setLoading(true)
      await JainFestivalService.delete(id)
      fetchData()
    } catch (error) {
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex gap-2'>
            {hasPermission('spiritual_jain_festival:edit') && <button onClick={() => {
            ...
            ></button>}
            {hasPermission('spiritual_jain_festival:delete') && <button
    ...
    <Grid className='' container>
      ...
      <Grid className='w-full' item xs={12}>
          ...
          <Card>
            <CardContent className='flex flex-col gap-4 sm:flex-row items-start sm:items-center justify-between flex-wrap'>
              ...
              <div className='flex flex-wrap gap-4'>
                ...
                />
                {hasPermission('spiritual_jain_festival:add') && <Button variant='contained' onClick={() => { setOpen(true); setEditValue(null) }}>
                  Add Festival
                </Button>}
              </div>
            </CardContent>
        ...
        <HinduFestivalModal
          ...
          folderName='Spiritual/Jainism/Festival'
          title='Festival Name'
        />
      </Grid>
    </Grid>
