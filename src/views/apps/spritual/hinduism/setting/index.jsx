...
const HinduSetting = () => {
    ...
    const handleCropConfirm = async (croppedImage) => {
        ...
        try {
            ...
            const uploaded = await ImageService.uploadImage(formData, { folder: "Spiritual/Hinduism/Setting" })
    ...
    const fetchSettings = async () => {
        try {
            const response = await hinduSettingService.get()
            ...
            if (response && response.data) {
                ...
                const mergedSettings = {
                    ...
                    hinduism_page: ensureArray(rest.hinduism_page, defaultImage),
                    chants: ensureArray(rest.chants, defaultChant),
                    featured: {
                        temple: { ...defaultImage, ...rest.featured?.temple },
                        aarti: { ...defaultImage, ...rest.featured?.aarti },
                        dharmik_gyaan: { ...defaultImage, ...rest.featured?.dharmik_gyaan },
                        live_darshan: { ...defaultImage, ...rest.featured?.live_darshan },
                        festival: { ...defaultImage, ...rest.featured?.festival },
                        articles: { ...defaultImage, ...rest.featured?.articles },
                    },
                    ad_slider: ensureArray(rest.ad_slider, { image: '', businessId: '' }),
                    temple: ensureArray(rest.temple, defaultImage),
                    aarti: ensureArray(rest.aarti, defaultImage),
                    dharmik_gyaan: ensureArray(rest.dharmik_gyaan, defaultImage),
                    festival: ensureArray(rest.festival, defaultImage),
                    articles: ensureArray(rest.articles, defaultImage),
    ...
    const onImageSelect = async (file, cropContext) => {
        ...
        } else if (['temple', 'aarti_chalisa', 'dharmik_gyaan', 'festival', 'articles', 'live_darshan'].includes(cropContext.section)) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await hinduSettingService.update(settings)
        } catch (error) {
    ...
    return (
        <Grid container spacing={2}>
            <Grid item xs={12}>
                <Typography variant="h5">Hindu Settings</Typography>
            </Grid>
            ...
            <Grid item xs={12}>
                <Card className="p-2">
                    <Grid container justifyContent={'space-between'} alignItems={'center'} className="mb-1">
                        <Typography variant="body1" className="font-semibold">Hinduism Page</Typography>
                        <Button
                            variant="contained"
                            size="small"
                            onClick={() => {
                                setSettings(prev => ({
                                    ...prev,
                                    hinduism_page: [...prev.hinduism_page, { ...defaultImage }]
                                }))
                            }}
                        >
                            Add Banner
                    ...
                    <Grid container spacing={1}>
                        {settings.hinduism_page.map((banner, bannerIndex) => (
                                ...
                                <Grid container spacing={1} alignItems="center">
                                    <Grid item xs={12}>
                                        <Grid container justifyContent={'space-between'} alignItems={'center'}>
                                            ...
                                            {settings.hinduism_page.length > 1 && (
                                                <IconButton
                                                    size="small"
                                                    color="error"
                                                    onClick={() => {
                                                        const updated = [...settings.hinduism_page]
                                                        updated.splice(bannerIndex, 1)
                                                        setSettings(prev => ({ ...prev, hinduism_page: updated }))
                                                    }}
                                                >
                                    ...
                                    {['web_image', 'mobile_image'].map(type => {
                                        const fieldKey = `hinduism_page_${bannerIndex}_${type}`;
                                        return (
            ...
            {/* Additional Pages */}
            {['temple', 'aarti', 'dharmik_gyaan', 'festival', 'articles', 'live_darshan'].map((section) => (
            ...
            {/* Submit Button */}
            <Grid item xs={12}>
                {hasPermission('spiritual_hinduism_setting:add') && <Button
                    variant="contained"
                    color="primary"
                    fullWidth
