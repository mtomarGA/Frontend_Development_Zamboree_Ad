...
const Security = ({ nextHandle, handleCancel }) => {
  ...
  const handleSendOTP = async (type) => {
    if (type === 'email') {
      await HinduService.sendEMailOTP({ email: securityFormData.email, name:"Gahram User" })
    } else {
      await HinduService.sendMobileOTP(securityFormData.phoneNumber)
    }
  ...
  const verifyOTP = async (otp, type) => {
    ...
    if (type === 'email') {
      const res = await HinduService.verifyEmailOTP({ otp, email: securityFormData.email })
      ...
      const res = await HinduService.verifyMobileOTP({ mobile: securityFormData.phoneNumber, otp })
    ...
    <Card sx={{ p: 4 }}>
      <Box component='form' onSubmit={handleFinalSubmit} noValidate>
        <Typography variant='h5' sx={{ mb: 4 }}>
          ...
          <Grid item xs={12} sm={6} md={4}>
            <CustomTextField select onChange={handleSecurityChange('status')} fullWidth label='Spiritual User Status' defaultValue={securityFormData.status}>
          ...
          <Grid item xs={12} sm={6} md={4}>
            <CustomTextField select onChange={handleSecurityChange('templeStatus')} fullWidth label='Temple Status' defaultValue={securityFormData.templeStatus}>
