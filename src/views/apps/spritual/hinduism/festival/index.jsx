...
const HinduismFestival = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const response = await HinduFestivalService.getAll()
  ...
  const handleAdd = async (values) => {
    try {
      setLoading(true)
      await HinduFestivalService.create(values)
      fetchData()
      setOpen(false)
    } catch (error) {
  ...
  const handleUpdate = async (values) => {
    try {
      setLoading(true)
      await HinduFestivalService.update(editValue._id, values)
      fetchData()
      setOpen(false)
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      setLoading(true)
      await HinduFestivalService.delete(id)
      fetchData()
    } catch (error) {
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex gap-2'>
            ...
            ></button> */}
            {hasPermission('spiritual_hinduism_festival:edit') && <button onClick={() => {
              setEditValue(row.original)
              setOpen(true)
            }}
            ...
            ></button>}
            {hasPermission('spiritual_hinduism_festival:delete') && <button
    ...
    <Grid className='' container>
      ...
      <Grid className='w-full' item xs={12}>
          ...
          <Card>
            <CardContent className='flex flex-col gap-4 sm:flex-row items-start sm:items-center justify-between flex-wrap'>
              ...
              <div className='flex flex-wrap gap-4'>
                ...
                />
                {hasPermission('spiritual_hinduism_festival:add') && <Button variant='contained' onClick={() => {setOpen(true); setEditValue(null)}}>
                  Add Festival
                </Button>}
              </div>
            </CardContent>
        ...
        <HinduFestivalModal
          ...
          handleAdd={handleAdd}
          folderName='Spiritual/Hinduism/Festival'
...
export default HinduismFestival;
