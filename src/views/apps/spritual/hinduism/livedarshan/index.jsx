...
const LiveDarshan = ({ handleAddEmployee }) => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const response = await HinduLiveDarshanService.getAll()
  ...
  const handleAdd = async (values) => {
    try {
      setLoading(true)
      await HinduLiveDarshanService.create(values)
      fetchData()
      setOpen(false)
    } catch (error) {
  ...
  const handleUpdate = async (values) => {
    try {
      setLoading(true)
      await HinduLiveDarshanService.update(editValue._id, values)
      fetchData()
      setOpen(false)
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      setLoading(true)
      await HinduLiveDarshanService.delete(id)
      fetchData()
    } catch (error) {
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor('temple.name', {
        header: 'Temple',
        cell: ({ row }) => <Typography color='text.primary'>{row.original.temple.name}</Typography>
      }),
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex gap-2'>
            ...
            {hasPermission('spiritual_hinduism_live_darshan:edit') && <button onClick={() => {
  ...
  return (
    <Grid className='' container>
      ...
      <Grid className='w-full' item xs={12}>
        ...
        <HinduLiveDarshanModal
          open={open}
          setOpen={setOpen}
          data={editValue}
          handleUpdate={handleUpdate}
          handleAdd={handleAdd}
          folderName='Spiritual/Hinduism/LiveDarshan'
