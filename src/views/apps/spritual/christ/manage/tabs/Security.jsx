...
const Security = ({ nextHandle, handleCancel }) => {
  ...
  const handleSendOTP = async (type) => {
    if (type === 'email') {
      await ChristService.sendEmailOTP({ email: securityFormData.email, name: "Gahram User" })
    } else {
      await ChristService.sendMobileOTP(securityFormData.phoneNumber)
    }
  ...
  const verifyOTP = async (otp, type) => {
    ...
    if (type === 'email') {
      const res = await ChristService.verifyEmailOTP({ email: securityFormData.email, otp })
      ...
      const res = await ChristService.verifyMobileOTP({ mobile: securityFormData.phoneNumber, otp })
