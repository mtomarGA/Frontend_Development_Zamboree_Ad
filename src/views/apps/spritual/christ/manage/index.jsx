...
import { useJainism, useJanism } from '@/contexts/JainFormContext'
...
import { useChrist } from '@/contexts/ChristFormContext'
...
const ChristManage = ({ handleAddEmployee }) => {
  // States
  ...
  const { handleLoadData, deleteEmployee, resetForm } = useChrist()
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const response = await ChristService.getAll()
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor('temple_id', {
        header: 'Temple ID',
        cell: ({ row }) => <Typography color='text.primary'>{row.original.temple_id || 'N/A'}</Typography>
      }),
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex gap-2'>
            ...
            ></button>
            {hasPermission('spiritual_christ_manage_church:edit') && <button
              onClick={() => {
                resetForm()
                // handleLoadData(row.original)
                router.push(`/en/apps/spritual/christ/manage/${row.original._id}`)
                // handleAddEmployee()
              }}
              className='text-blue-600  tabler-edit text-xl cursor-pointer'
            ></button>}
            {hasPermission('spiritual_christ_manage_church:delete') && <button
          ...
          <Card>
            <CardContent className='flex flex-col gap-4 sm:flex-row items-start sm:items-center justify-between flex-wrap'>
              ...
              <div className='flex flex-wrap gap-4'>
                ...
                />
                {hasPermission('christ_manage_church:add') && <Button variant='contained' onClick={() => router.push(`/en/apps/spritual/christ/manage/create`)}>
                  Add Church
                </Button>}
              </div>
            </CardContent>
