...
import ChristSettingService from '@/services/spritual/christSetting'
...
const Setting = () => {
    ...
    const handleCropConfirm = async (croppedImage) => {
        ...
        try {
            ...
            const uploaded = await ImageService.uploadImage(formData, { folder: `Spiritual/Christianity/Setting` })
    ...
    const fetchSettings = async () => {
        try {
            const response = await ChristSettingService.get()
            ...
            if (response && response.data) {
                ...
                const mergedSettings = {
                    ...
                    christ_page: ensureArray(rest.christ_page, defaultImage),
    ...
    const onImageSelect = async (file, cropContext) => {
        ...
        } else if (['temple', 'aarti', 'dharmik_gyaan', 'festival', 'articles', 'live_darshan'].includes(cropContext.section)) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await ChristSettingService.update(settings)
    ...
    return (
        <Grid container spacing={2}>
            <Grid item xs={12}>
                <Typography variant="h4">Christian Settings</Typography>
            </Grid>
            ...
            <Grid item xs={12}>
                <Card className="p-2">
                    <Grid container justifyContent={'space-between'} alignItems={'center'} className="mb-1">
                        ...
                        <Button
                            ...
                            onClick={() => {
                                setSettings(prev => ({
                                    ...prev,
                                    christ_page: [...prev.christ_page, { ...defaultImage }]
                                }))
                            }}
                        >
                            Add Banner
                    ...
                    <Grid container spacing={1}>
                        {settings.christ_page.map((banner, bannerIndex) => (
                                ...
                                <Grid container spacing={1} alignItems="center">
                                    <Grid item xs={12}>
                                        <Grid container justifyContent={'space-between'} alignItems={'center'}>
                                            ...
                                            {settings.christ_page.length > 1 && (
                                                <IconButton
                                                    size="small"
                                                    color="error"
                                                    onClick={() => {
                                                        const updated = [...settings.christ_page]
                                                        updated.splice(bannerIndex, 1)
                                                        setSettings(prev => ({ ...prev, christ_page: updated }))
                                                    }}
                                                >
                                    ...
                                    {['web_image', 'mobile_image'].map(type => {
                                        const fieldKey = `christ_page_${bannerIndex}_${type}`;
                                        return (
            ...
            {/* Submit Button */}
            <Grid item xs={12}>
                {hasPermission('spiritual_christ_setting:add') && <Button
                    variant="contained"
                    color="primary"
                    fullWidth
