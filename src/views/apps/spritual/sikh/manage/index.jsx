...
const SikhismManage = ({ handleAddEmployee }) => {
  // States
  ...
  const { handleLoadData, deleteEmployee, resetForm } = useGurudwara()
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const response = await SikhService.getAllGurudwaras()
  ...
  const handleDelete = async (id) => {
    try {
      await SikhService.deleteGurudwara(id)
      fetchData()
    } catch (error) {
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor('gurudwara_id', {
        header: 'Gurudwara ID',
        cell: ({ row }) => <Typography color='text.primary'>{row.original.gurudwara_id || "N/A"}</Typography>
      }),
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex gap-2'>
            ...
            ></button>
            {hasPermission('spiritual_sikh_manage_gurudwara:edit') && <button
              onClick={() => {
                resetForm()
                // handleLoadData(row.original)
                router.push(`/en/apps/spritual/sikh/manage/${row.original._id}`)
                // handleAddEmployee()
              }}
              className='text-blue-600  tabler-edit text-xl cursor-pointer'
            ></button>}
            {hasPermission('spiritual_sikh_manage_gurudwara:delete') && <button
          ...
          <Card>
            <CardContent className='flex flex-col gap-4 sm:flex-row items-start sm:items-center justify-between flex-wrap'>
              ...
              <div className='flex flex-wrap gap-4'>
                ...
                />
                {hasPermission('spiritual_sikh_manage_gurudwara:add') && <Button variant='contained' onClick={() => router.push(`/en/apps/spritual/sikh/manage/create`)}>
                  Add Gurudwara
                </Button>}
              </div>
            </CardContent>
