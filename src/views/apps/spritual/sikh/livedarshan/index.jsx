...
import SikhLiveDarshanService from '@/services/spritual/sikhLiveDarshan'
import SikhLiveDarshanModal from '@/components/dialogs/sikh-live-darshan'
...
const LiveDarshan = ({ handleAddEmployee }) => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const response = await SikhLiveDarshanService.getAll()
  ...
  const handleAdd = async (values) => {
    try {
      setLoading(true)
      await SikhLiveDarshanService.create(values)
      fetchData()
      setOpen(false)
    } catch (error) {
  ...
  const handleUpdate = async (values) => {
    try {
      setLoading(true)
      await SikhLiveDarshanService.update(editValue._id, values)
      fetchData()
      setOpen(false)
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      setLoading(true)
      await SikhLiveDarshanService.delete(id)
      fetchData()
    } catch (error) {
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor('gurudwara.name', {
        header: 'Gurudwara',
        cell: ({ row }) => <Typography color='text.primary'>{row.original.gurudwara.name || 'N/A'}</Typography>
      }),
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex gap-2'>
            ...
            {hasPermission('spiritual_sikh_live_darshan:edit') && <button onClick={() => {
            ...
            ></button>}
            {hasPermission('spiritual_hinduism_live_darshan:delete') && <button onClick={() => handleDelete(row.original._id)}
    ...
    <Grid className='' container>
      ...
      <Grid className='w-full' item xs={12}>
          ...
          <Card>
            <CardContent className='flex flex-col gap-4 sm:flex-row items-start sm:items-center justify-between flex-wrap'>
              ...
              <div className='flex flex-wrap gap-4'>
                ...
                />
                {hasPermission('spiritual_hinduism_live_darshan:add') && <Button variant='contained' onClick={() => { setOpen(true); setEditValue(null) }}>
                  Add Live Darshan
                </Button>}
              </div>
            </CardContent>
        ...
        <LiveDarshanModal
          ...
          handleUpdate={handleUpdate}
          handleAdd={handleAdd}
          folderName='Spiritual/Sikhism/LiveDarshan'
          handleClose={() => setOpen(false)}
          title='Live Darshan'
          serviceName="Sikhism"
        />
