...
import sikhContentService from '@/services/spritual/sikhContent'
...
const ManageSikhContent = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const response = await sikhContentService.getAll()
  ...
  const handleDelete = async (id) => {
    try {
      setLoading(true)
      await sikhContentService.delete(id)
      fetchData()
    } catch (error) {
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex gap-2'>
            ...
            {hasPermission('spiritual_sikh_manage_content:edit') && <button onClick={() => router.push(`/en/apps/spritual/sikh/content/${row.original._id}`)}
              className='text-blue-600  tabler-edit text-xl cursor-pointer'
            ></button>}
            {hasPermission('spiritual_sikh_manage_content:view') && <button onClick={() => {
              setSelectedData(row.original)
              setOpen(true)
            }}
              className='text-green-600 tabler-eye text-xl cursor-pointer'
            ></button>}
            {hasPermission('spiritual_hinduism_manage_content:delete') && <button onClick={() => handleDelete(row.original._id)}
          ...
          <Card>
            <CardContent className='flex flex-col gap-4 sm:flex-row items-start sm:items-center justify-between flex-wrap'>
              ...
              <div className='flex flex-wrap gap-4'>
                ...
                />
                {hasPermission('spiritual_hinduism_manage_content:add') && <Button variant='contained' onClick={() => router.push('/en/apps/spritual/sikh/content/new')}>
                  Add Content
                </Button>}
              </div>
            </CardContent>
...
export default ManageSikhContent;
