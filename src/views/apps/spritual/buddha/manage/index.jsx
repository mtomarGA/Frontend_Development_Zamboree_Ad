...
const BuddhismManage = ({ handleAddEmployee }) => {
  // States
  ...
  const { handleLoadData, deleteEmployee, resetForm } = useBuddhism()
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const response = await BuddhismService.getAll()
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex gap-2'>
            ...
            ></button>
            {hasPermission('spiritual_buddhism_manage_temple:edit') && <button
              onClick={() => {
                resetForm()
                // handleLoadData(row.original)
                router.push(`/en/apps/spritual/buddha/manage/${row.original._id}`)
                // handleAddEmployee()
              }}
              className='text-blue-600  tabler-edit text-xl cursor-pointer'
            ></button>}
            {hasPermission('spiritual_buddhism_manage_temple:delete') && <button
          ...
          <Card>
            <CardContent className='flex flex-col gap-4 sm:flex-row items-start sm:items-center justify-between flex-wrap'>
              ...
              <div className='flex flex-wrap gap-4'>
                ...
                />
                {hasPermission('spiritual_buddhism_manage_temple:add') && <Button variant='contained'onClick={() => router.push(`/en/apps/spritual/buddha/manage/create`)}>
                  Add Buddhism Temple
                </Button>}
              </div>
            </CardContent>
