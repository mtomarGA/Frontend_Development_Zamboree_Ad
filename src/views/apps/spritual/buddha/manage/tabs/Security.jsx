...
const Security = ({ nextHandle, handleCancel }) => {
  ...
  const handleSendOTP = async (type) => {
    if (type === 'email') {
      await BuddhismService.sendEmailOTP({ email: securityFormData.email, name: "Gahram User" })
    } else {
      await BuddhismService.sendMobileOTP(securityFormData.phoneNumber)
    }
  ...
  const verifyOTP = async (otp, type) => {
    ...
    if (type === 'email') {
      const res = await BuddhismService.verifyEmailOTP({ email: securityFormData.email, otp })
      ...
      const res = await BuddhismService.verifyMobileOTP({ mobile: securityFormData.phoneNumber, otp })
