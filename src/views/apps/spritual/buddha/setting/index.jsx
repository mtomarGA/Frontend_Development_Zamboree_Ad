...
import BuddhismSettingService from '@/services/spritual/buddhaSetting'
...
const Setting = () => {
    ...
    const fetchSettings = async () => {
        try {
            const response = await BuddhismSettingService.get()
            ...
            if (response && response.data) {
                ...
                const mergedSettings = {
                    ...
                    buddhism_page: ensureArray(rest.buddhism_page, defaultImage),
                    ...
                    aarti_chalisa: ensureArray(rest.aarti_chalisa, defaultImage),
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await BuddhismSettingService.update(settings)
    ...
    return (
        <Grid container spacing={2}>
            <Grid item xs={12}>
                <Typography variant="h4">Buddhism Settings</Typography>
            </Grid>
            ...
            <Grid item xs={12}>
                <Card className="p-2">
                    <Grid container justifyContent={'space-between'} alignItems={'center'} className="mb-1">
                        <Typography variant="body1" className="font-semibold">Buddhism Page</Typography>
                        <Button
                            variant="contained"
                            size="small"
                            onClick={() => {
                                setSettings(prev => ({
                                    ...prev,
                                    buddhism_page: [...prev.buddhism_page, { ...defaultImage }]
                                }))
                            }}
                        >
                            Add Banner
                    ...
                    <Grid container spacing={1}>
                        {settings.buddhism_page.map((banner, bannerIndex) => (
                                ...
                                <Grid container spacing={1} alignItems="center">
                                    <Grid item xs={12}>
                                        <Grid container justifyContent={'space-between'} alignItems={'center'}>
                                            ...
                                            {settings.buddhism_page.length > 1 && (
                                                <IconButton
                                                    size="small"
                                                    color="error"
                                                    onClick={() => {
                                                        const updated = [...settings.buddhism_page]
                                                        updated.splice(bannerIndex, 1)
                                                        setSettings(prev => ({ ...prev, buddhism_page: updated }))
                                                    }}
                                                >
                                    ...
                                    {['web_image', 'mobile_image'].map(type => {
                                        const fieldKey = `buddhism_page_${bannerIndex}_${type}`;
                                        return (
            ...
            {/* Additional Pages */}
            {['temple', 'aarti_chalisa', 'dharmik_gyaan', 'festival', 'articles', 'live_darshan'].map((section) => (
            ...
            {/* Submit Button */}
            <Grid item xs={12}>
                {hasPermission('spiritual_buddhism_setting:add') && <Button
                    variant="contained"
                    color="primary"
                    fullWidth
