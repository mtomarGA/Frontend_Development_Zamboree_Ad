...
function ZoneTable({ handleClickOpen, GetLevelFun, getlevel }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await QuizQuestionRoute.Post(options)
            if (!response.success) {
    ...
    const deleteid = async (id) => {
        const response = await QuizLevelRoute.deleteData(id)
        if (response.success === true) {
            toast.success("Quiz Level Deleted Successfully")
            GetLevelFun()
            return
        }
    ...
    return (
        <div>
            ...
            {/* edit modal */}
            <EditModal
                levelEditOpen={levelEditOpen}
                handlelevelEditClose={handlelevelEditClose}
                GetLevelFun={GetLevelFun}
            ...
            <TableContainer className='shadow p-6'>
                <div className='flex justify-between items-center mb-4'>
                    <h3 className='mb-4'>Quiz Zone Manage Level</h3>
                    ...
                    <TableBody>
                        {paginatedData.map((row) => (
                            <TableRow key={row?._id} className='border-b'>
                                ...
                                <TableCell className='p-2'>{row?.maxQues ? `${row?.maxQues} Questions` : '0'}</TableCell>
                                <TableCell className='p-2'>{row?.entry_fee ? `${row?.entry_fee} Coins` : '0 Coins'}</TableCell>
                                ...
                                <TableCell className='p-2'>
                                    <Link href={`/${lang}/apps/quiz/quizzone/question/${row._id}`}>
                                ...
                                <TableCell className='p-2 flex items-center'>
                                    {hasPermission('quiz_quiz_zone:edit') && (
                                        <Pencil
                                            ...
                                            onClick={() => handlelevelEditOpen(row)}
                ...
                <div className='flex flex-col sm:flex-row justify-between items-center m-4 gap-4'>
                    <Typography variant='body2' className='text-gray-600'>
                        ...
                        {Math.min(currentPage * rowsPerPage, getlevel.length)} of {getlevel.length} entries
                    </Typography>
                    <PaginationRounded
