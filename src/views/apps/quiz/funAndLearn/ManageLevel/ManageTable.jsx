...
function ZoneTable({ handleClickOpen, GetLevelFun, getlevel }) {
    ...
    const [options, setOptions] = useState({ A: '', B: '', C: '', D: '', answer: '', question: '', questionType: 'options' })
    ...
    const handleQuesOpen = (id) => {
        setQuesOpen(true)
        setOptions({ A: '', B: '', C: '', D: '', answer: '', question: '', level: id, questionType })
    }
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await FunQuesService.Post(options)
            if (!response.success) {
    ...
    const deleteid = async (id) => {
        const response = await FunAndLearnLevelRoute.deleteData(id);
        ...
        <div>
            ...
            <TableContainer className='shadow p-6'>
                <div className='flex justify-between items-center mb-4'>
                    <h3 className='mb-4'>Fun & Learn Manage Level</h3>
                    ...
                    <TableBody>
                        {paginatedData.map((row) => (
                            <TableRow key={row._id} className='border-b'>
                                ...
                                <TableCell className='p-2'>
                                    {hasPermission('quiz_funAndLearn:view') && (

                                        <Link href={`/${lang}/apps/quiz/funAndLearn/managelevel/question/${row._id}`}>
                                            <Eye className='text-blue-500 size-5 mx-2 cursor-pointer hover:scale-110 transition' />
                                        </Link>
                                    )}
                                </TableCell>
                                <TableCell className='p-2'>
                                    {hasPermission('quiz_manage_level:add') && (
                                ...
                                <TableCell className='p-2 flex items-center'>
                                    ...
                                    {hasPermission('quiz_funAndLearn:delete') &&
                                        <Trash2
