...
const EditModal = ({ levelEditOpen, handlelevelEditClose, GetLevelFun, selectedLevel }) => {
    ...
    const validateFields = (data) => {
        ...
        if(!data.free_hints) errors.free_hints = 'Free Hint is required'
        if(!data.hint_deduct_Coins) errors.hint_deduct_Coins = 'Hint Deduct Coins is required'
    ...
    const fetchcategory = async () => {
        const res = await GuesTheWordCategoryRoute.category();
    ...
    const fetchsubcategory = async () => {
        const res = await GuesTheWordSubCategoryRoute.subcategory(EditData?.category || []);
    ...
    useEffect(() => {
        if (selectedLevel) {
            setEditData({
                ...
                ques_duration: selectedLevel?.ques_duration || '',
                scorePerCorrectAns: selectedLevel?.scorePerCorrectAns || '',
                free_hints: selectedLevel?.free_hints || '',
                hint_deduct_Coins: selectedLevel?.hint_deduct_Coins || '',
                status: selectedLevel?.status || 'INACTIVE'
            });
        }
    }, [selectedLevel]);
    ...
    const handleSubmit = async () => {
        ...
        const response = await GuesTheWordLevelRoute.putData(selectedLevel?._id, EditData)
                ...
                <DialogContent>
                    ...
                    <div className='m-2'>
                        <CustomTextField
                            ...
                            onChange={handleChange}
                            value={EditData.free_hints || ''}
                            multiline
                            rows={1}
                            variant='outlined'
                            error={!!formErrors.free_hints}
                            helperText={formErrors.free_hints}
                        />
                    </div>
                    <div className='m-2'>
                        <CustomTextField
                            className='w-96'
                            name='hint_deduct_Coins'
                            label='Hint Deduct Coins'
                            type='number'
                            inputProps={{ inputMode: 'numeric', pattern: '[0-9]*', min: 0 }}
                            onKeyDown={(e) => {
                                if (['e', 'E', '+', '-'].includes(e.key)) e.preventDefault()
                            ...
                            onChange={handleChange}
                            value={EditData.hint_deduct_Coins || ''}
                            multiline
                            rows={1}
                            variant='outlined'
                            error={!!formErrors.hint_deduct_Coins}
                            helperText={formErrors.hint_deduct_Coins}
                        />
                    </div>
