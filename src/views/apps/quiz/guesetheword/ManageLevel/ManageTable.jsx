...
function ZoneTable({ handleClickOpen, GetLevelFun, getlevel }) {
    ...
    const [options, setOptions] = useState({ answer: '', question: '' })
    ...
    const handleQuesOpen = (id) => {
        setQuesOpen(true)
        setOptions({ answer: '', question: '', level: id, })
    }
    ...
    const handlelevelEditOpen = (row) => {
        ...
        // setOptions({ A: '', B: '', C: '', D: '', answer: '', question: '', level: id })
    }
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await GuesTheWordQuesService.Post(options)
            if (!response.success) {
    ...
    const deleteid = async (id) => {
        const response = await GuesTheWordLevelRoute.deleteData(id);
        if (response.success === true) {
            toast.success("Fun & Learn Level Deleted Successfully");
            GetLevelFun();
            return;
        }
    ...
    return (
        ...
        <div>
            <Dialog open={QuesOpen} fullWidth onClose={handleQuesClose} PaperProps={{ sx: { overflow: 'visible' } }}>
                <DialogTitle>
                ...
                <DialogContent>
                    ...
                    {/* <Box mb={2}>
                        ...
                        <RadioGroup row value={questionType} onChange={(e) => {
                            setQuestionType(e.target.value)
                            setOptions(prev => ({ ...prev, questionType: e.target.value }))
                        }}>
                    ...
                    <Box mt={2}>
                        <Typography variant='subtitle1'>Image for Question</Typography>
                        <input
            ...
            <TableContainer className='shadow p-6'>
                <div className='flex justify-between items-center mb-4'>
                    <h3 className='mb-4'>Guess The Word Manage Level</h3>
                    ...
                    <TableBody>
                        {paginatedData.map((row) => (
                            <TableRow key={row._id} className='border-b'>
                                ...
                                <TableCell className='p-2'>
                                    {hasPermission('quiz_guesstheword:view') && (

                                        <Link href={`/${lang}/apps/quiz/guesetheword/managelevel/question/${row._id}`}>
