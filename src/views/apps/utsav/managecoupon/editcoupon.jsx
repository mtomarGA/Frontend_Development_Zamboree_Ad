...
import { Avatar, Card, Checkbox, CircularProgress, FormControl, FormGroup, FormLabel, InputAdornment, MenuItem, TextField, Typography } from '@mui/material'
...
const AddcouponModal = () => {
  ...
  // const [discountType, setDiscountType] = useState(addcoupondata.discountType || '')
  ...
  //   // console.log(result.data, "bbbbbbbbbbb")
  ...
  const validateFields = data => {
    ...
    // Validate discount fields conditionally
    ...
    if (!data.description[0]) errors.description = 'Deal Description is required'
    ...
    if (!data.how_to_redeem) errors.how_to_redeem = 'How to redeem is required'
    if (!data.terms[0]) errors.terms = 'Terms & Conditions are required'
  ...
  useEffect(() => {
    const editid = async () => {
      ...
      // Convert images from old format to new format if needed
      let imagesArray = []
      if (data.images && Array.isArray(data.images)) {
        if (data.images.length > 0) {
          // Check if it's the new format (array of strings) or old format (array of objects)
          if (typeof data.images[0] === 'string') {
            // New format: array of URLs
            imagesArray = data.images.filter(img => img && typeof img === 'string')
          } else if (typeof data.images[0] === 'object' && data.images[0] !== null) {
            // Old format: array of objects with webimage, mobimage, thumbnailimage
            data.images.forEach(img => {
              if (img && typeof img === 'object') {
                if (img.webimage) imagesArray.push(img.webimage)
                if (img.mobimage) imagesArray.push(img.mobimage)
                if (img.thumbnailimage) imagesArray.push(img.thumbnailimage)
      ...
      setFields(data.how_to_redeem ?? ['']);
      settermFields(data.terms || ['']);

      // fetch dependent dropdowns
      // if (data.country?._id) states(data.country._id)
      // if (data.state?._id) citys(data.state._id)
      // if (data.city?._id) area(data.city._id)
    }
  ...
  const SubmitCoupon = async () => {
    const finalData = { ...addcoupondata, how_to_redeem: fields }

    const errors = validateFields(finalData)
    ...
    setFormErrors({})
    const response = await CouponRoute.putData(id, finalData)
    toast.success('Coupon Updated');
    redirect('/apps/utsav/managecoupon')
  }
  ...
  // useEffect(() => {
  //   if (addcoupondata.fixedvalue) {
  //     setDiscountType('fixed');
  //   } else if (addcoupondata.percentage) {
  //     setDiscountType('percentage');
  //   } else {
  //     setDiscountType(''); // Reset if none
  //   }
  // }, [addcoupondata.fixedvalue, addcoupondata.percentage]);
        ...
        <div className="flex flex-col lg:flex-row justify-between gap-6">
          ...
          <div className="flex-1 space-y-4">
            ...
            <div >
              <CustomTextField
                ...
                className='w-full'
                label='Business ID'
                value={addcoupondata?.BusinessId?.vendorId || ''}
                multiline
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <CustomTextField
                ...
                value={addcoupondata.vouchertype?._id || addcoupondata.vouchertype || ''}
              ...
              >
                {[...new Map(voucherTypes.map(item => [item.vouchertype, item])).values()]
              ...
              {/* <CustomTextField
                ...
                id='controlled-select'
                value={addcoupondata.category?._id || addcoupondata.category || ''}
          ...
          <div className="w-full lg:w-[30%]">
            ...
            <div className="border-2 rounded shadow border-gray-600 p-4 space-y-1 text-sm">
              <div><strong>Parent ID:</strong> {addcoupondata.BusinessId?.vendorId || 'N/A'}</div>
              <div><strong>Business Name:</strong> {addcoupondata.BusinessId?.companyInfo?.companyName || 'N/A'}</div>
              <div><strong>Country:</strong>{addcoupondata.BusinessId?.locationInfo?.country || 'N/A'}</div>
              <div><strong>State:</strong> {addcoupondata.BusinessId?.locationInfo?.state || 'N/A'}</div>
              <div><strong>City:</strong> {addcoupondata.BusinessId?.locationInfo?.city || 'N/A'}</div>
              <div><strong>Area:</strong> {addcoupondata.BusinessId?.locationInfo?.area || 'N/A'}</div>
            </div>
          </div>
        </div>
        ...
        <div className='flex flex-wrap justify-between sm:w-[80%]'>
          <div className='m-4'>
            <Grid item xs={12} md={4}>
              <AppReactDatepicker
                selected={
                  (addcoupondata.expiryDate ? new Date(addcoupondata.expiryDate) : date)
                }
