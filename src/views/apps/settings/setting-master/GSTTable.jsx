...
import GstPriceForm from '@/components/dialogs/settings/settingMaster/gstmaster/GST_Master'
...
const GSTTable = () => {
    ...
    const [EditSelectedGST, setEditSelectedGST] = useState("")
    ...
    const columns = useMemo(() => {
        ...
        return [
            columnHelper.accessor('Percentage Price', {
                header: 'Percentage Price',
                cell: ({ row }) =>

                (
                    <Typography className='font-medium' color='text.primary'>
                        {row.original?.gstPrice}%
                    </Typography>
                )
            }),
            columnHelper.accessor('mssage', {
                header: 'GST Message',
                cell: ({ row }) => (
                    <Typography className='font-medium' color='text.primary'>
                ...
                cell: ({ row }) => (
                    <div className='flex items-center gap-2' >
                        <IconButton
                            onClick={() => {
                                setADDGSTOpen(true)
                                setEditSelectedGST(row.original)
                            }}
                        >
    ...
    return (
        <Card className='shadow-none '>
            ...
            <Dialog fullWidth open={ADDGSTOpen} maxWidth='sm' scroll='body' sx={{ '& .MuiDialog-paper': { overflow: 'visible' } }}>
                <DialogCloseButton onClick={() => setADDGSTOpen(false)} disableRipple>
                    <i className='tabler-x' />
                </DialogCloseButton>
                <DialogTitle variant='h4' className='flex gap-2 flex-col text-center   '>
                    <GstPriceForm gstTextService={gstTextService} EditSelectedGST={EditSelectedGST} setModalOpen={setADDGSTOpen} />
                </DialogTitle>
            </Dialog>
        </Card>
    )
