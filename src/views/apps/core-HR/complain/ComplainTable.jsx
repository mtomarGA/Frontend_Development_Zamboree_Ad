...
const ComplainTable = () => {
  ...
  const getAllComplain = async () => {
    const res = await complaintServices.getComplaint()
    setDate(res.data)
  }
  ...
  const confirmDelete = async () => {
    if (deleteId) {
      await complaintServices.deleteComplaint(deleteId)
      getAllComplain()
  ...
  const columns = useMemo(() => {
    ...
    return [
      columnHelper.accessor('complaintFrom.name', {
        header: 'Complaint From',
        cell: ({ row }) => (
          <Typography className='font-medium' color='text.primary'>
            {row.original?.complaintFrom?.name}
          </Typography>
        )
      }),
      columnHelper.accessor('complaintAgainst.name', {
        header: 'Complaint Against',
        cell: ({ row }) => (
          <Typography className='font-medium' color='text.primary'>
            {row.original?.complaintAgainst?.name}
          </Typography>
        ),

      }),
      columnHelper.accessor('complainType.name', {
        header: 'Complain Type',
        cell: ({ row }) => (
          <Typography className='font-medium' color='text.primary'>
            {row.original?.complainType?.name}
          </Typography>
        ),
      }),
      ...
      columnHelper.accessor('actions', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-2'>
            ...
            {hasPermission("core HR_complaints:view") && <IconButton onClick={() => {
              setComplaintData(row.original)
              setShowData(true)
            }}>
            ...
            {hasPermission("core HR_complaints:edit") && user?.userType === "Employee" && (
              <IconButton
                onClick={() => {
                  setComplaintData(row.original);
                  setModalOpen(true);
            ...
            {hasPermission("core HR_complaints:delete") && <IconButton
              onClick={() => {
  ...
  return (
    ...
    <Card>
      <Typography variant='h3' className='px-7 py-2'>Complain List</Typography>
      <div className='flex flex-wrap justify-between gap-4 p-6'>
        ...
        <div className='flex flex-wrap items-center max-sm:flex-col gap-4 max-sm:is-full is-auto'>
          ...
          {hasPermission("core HR_complaints:add") && user?.userType === "Employee" && (
            <Button
              variant="contained"
      ...
      <Dialog fullWidth open={ModalOpen} maxWidth='sm' scroll='body' sx={{ '& .MuiDialog-paper': { overflow: 'visible' } }}>
        ...
        <DialogTitle variant='h4' className='flex gap-2 flex-col text-center sm:pbs-16 sm:pbe-6 sm:pli-16'>
          <ComplaintForm setModalOpen={setModalOpen} getAllComplain={getAllComplain} complaintData={complaintData} />
        </DialogTitle>
      </Dialog>
