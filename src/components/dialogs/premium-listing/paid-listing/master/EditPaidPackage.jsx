...
import PackagePaidService from '@/services/premium-listing/paidPackage.service';

const EditPaidPackageListing = ({ onsuccess, getPackage, EditSelectedPackage }) => {
    ...
    useEffect(() => {
        if (EditSelectedPackage) {
            setFormData({
                name: EditSelectedPackage.name || '',
                status: EditSelectedPackage.status || 'ACTIVE',
                description: EditSelectedPackage.description || '',
                price: EditSelectedPackage.price?.toString() || '',
                valideDate:EditSelectedPackage.valideDate||''
            });
        }
    }, [EditSelectedPackage]);
    ...
    const handleSave = async () => {
        ...
        if (!hasError) {
            const packageId = EditSelectedPackage?._id
            const result = await PackagePaidService.updatePaidPackage(packageId, formData)
            onsuccess(false)
            getPackage()
    ...
    return (
        <Card className='shadow-none'>
            <CardHeader title='Edit Paid Package' />
            <CardContent>
...
export default EditPaidPackageListing;
