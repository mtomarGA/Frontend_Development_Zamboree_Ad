...
import AllCOmment from '@/services/posts/comment.service'
import getAllReport from '@/services/posts/report.service'
import { toast } from 'react-toastify'
import ViewCommentReport from '@/components/dialogs/posts/report/ViewCommentReport'
...
const CommentReport = () => {
    ...
    const getusers = async () => {
        const res = await getAllReport.getPostCommentReport()
    ...
    const columns = useMemo(() => {
        ...
        return [
            ...
            columnHelper.accessor('userId', {
                header: 'Reported By',
                cell: ({ row }) => {
                    return <Typography>{row?.original?.userId?.userId }</Typography>
                }
            }),
            ...
            columnHelper.accessor('actions', {
                ...
                cell: ({ row }) => (
                    <div className='flex items-center gap-2'>
                        <IconButton
                            onClick={() => {
                                setViewSelectedReport(row.original)
        ...
        <Card className='shadow-none'>
            <div className='flex flex-wrap justify-between gap-4 p-6'>
                <div className='flex flex-wrap items-center gap-4'>
                    <CardHeader title='Reported Comment Table' />
                </div>
                <div className='flex flex-wrap items-center gap-4'>
                    <DebouncedInput
                        value={globalFilter ?? ''}
           ...
           <Dialog fullWidth open={EditModalOpen} maxWidth='lg' scroll='body' sx={{ '& .MuiDialog-paper': { overflow: 'visible' } }}>
                ...
                <DialogTitle variant='h4' className='flex gap-2 flex-col text-center'>
                  <ViewCommentReport ViewSelectedReport={ViewSelectedReport}  setEditModalOpen={setEditModalOpen} onSuccess={getusers}/>
                </DialogTitle>
            </Dialog>
        </Card>
    )
