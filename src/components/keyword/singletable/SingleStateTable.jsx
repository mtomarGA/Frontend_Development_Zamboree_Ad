...
const SingleStateTable = ({ keywordDataState,onSuccess,handleCitySearch}) => {
  ...
  const columns = useMemo(() => {
    ...
    return [
      columnHelper.accessor('state', {
        ...
        cell: ({ row }) =>{
          return (
            <Typography className='font-medium cursor-pointer hover:underline hover:text-[#7367F0] text-blue-400' color='text.primary'>
  ...
  const table = useReactTable({
    data : keywordDataState?.hitsByState,
    columns,
          ...
          <tbody>
            {table?.getRowModel().rows.length > 0 ? (
              table.getRowModel().rows.map(row => (
                <tr key={row.id} className={classnames({ selected: row.getIsSelected() })} onClick={()=>handleCitySearch(row?.original)} >
...
export default SingleStateTable
