...
// Recursively flatten menuConfig into searchable items with section info
const flattenMenu = (menu, parentSection = null) => {
  let items = []
  menu.forEach(item => {
    if (item.type === 'item' && item.label && item.href) {
      items.push({
        ...
        section: parentSection || 'Other'
      })
    } else if (item.type === 'submenu' && item.children && item.children.length > 0) {
      // Add submenu as a section if it has a label
      const section = item.label || parentSection
      items = items.concat(flattenMenu(item.children, section))
    }
  })
  return items
}
...
const NavSearch = () => {
  ...
  // Flatten menuConfig and filter by permissions
  const menuItems = flattenMenu(menuConfig)
  // Group by section, filter by permissions
  const groupBySection = (items, hasPermission) => {
    return items.reduce((acc, item) => {
      if (item.permissions && Array.isArray(item.permissions) && item.permissions.length > 0) {
        if (!item.permissions.some(p => hasPermission(p))) return acc
      }
  ...
  const transformedData = groupBySection(menuItems, hasPermission)
  ...
  return (
    <>
      {isBreakpointReached || settings.layout === 'horizontal' ? (
        ...
        <div className='flex items-center gap-2 w-auto cursor-pointer' onClick={() => setOpen(true)}>
          <IconButton className='text-textPrimary' onClick={() => setOpen(true)}>
            <i className='tabler-search text-2xl' />
          </IconButton>
