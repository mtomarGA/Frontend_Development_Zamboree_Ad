...
// --- Create Context ---
const GurudwaraFormContext = createContext();

// --- Create Provider Component ---
export const GurudwaraFormProvider = ({ children, onSuccess }) => {
    const router = useRouter()
    const [formData, setFormData] = useState(initialFormData)
    const [errors, setErrors] = useState(getInitialErrors(initialFormData))

    const [isEditMode, setIsEditMode] = useState(false);
    const [editGurudwaraId, seteditGurudwaraId] = useState('')
    ...
    const [gurudwaraTabManager, setGurudwaraTabManager] = useState({
        overview: true,
        timing: false,
        socialmedia: false,
        gallery: false,
    ...
    const validate = () => {
        ...
        if (!formData.gurudwara_name?.trim()) {
            newErrors.gurudwara_name = 'Gurudwara name is required.'
        }
        ...
        if (formData.about_gurudwara?.length <= 0) {
            newErrors.about_gurudwara = 'Additional section is required.'
        }
    ...
    const handleLoadData = async (data) => {
        ...
        setFormData({
            ...
            about_gurudwara: data.additional_info || '',
    ...
    const handleSubmit = async () => {
        ...
        if (isEditMode) {
            const ress = await SikhService.updateGurudwara(editGurudwaraId, body);
            console.log('Response:', ress);
        } else {
            const ress = await SikhService.createGurudwara(body);
            console.log('Response:', ress);
        }
        setGurudwaraTabManager({
            overview: true,
        ...
        router.push("/en/apps/spritual/sikh/manage")
        if (onSuccess) {
    ...
    const value = {
        ...
        gurudwaraTabManager, setGurudwaraTabManager

    };

    return (
        <GurudwaraFormContext.Provider value={value}>
            {children}
        </GurudwaraFormContext.Provider>
    );
};

// --- Custom Hook for Consuming Context ---
export const useGurudwara = () => {
    const context = useContext(GurudwaraFormContext);
    if (context === undefined) {
        throw new Error('useGurudwaraContext must be used within a GurudwaraFormProvider');
    }
    return context;
};
