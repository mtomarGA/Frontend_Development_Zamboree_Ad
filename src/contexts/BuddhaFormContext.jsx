...
// --- Create Context ---
const BuddhismFormContext = createContext();

// --- Create Provider Component ---
export const BuddhismFormProvider = ({ children, onSuccess }) => {
    ...
    const handleSubmit = async () => {
        ...
        if (isEditMode) {
            const ress = await BuddhismService.update(editTempleId, body);
            console.log('Response:', ress);
        } else {
            const ress = await BuddhismService.create(body);
            console.log('Response:', ress);
        }
        setTempleTabManager({
            overview: true,
        ...
        router.push("/en/apps/spritual/buddha/manage")
    }
    ...
    return (
        <BuddhismFormContext.Provider value={value}>
            {children}
        </BuddhismFormContext.Provider>
    );
};

// --- Custom Hook for Consuming Context ---
export const useBuddhism = () => {
    const context = useContext(BuddhismFormContext);
    if (context === undefined) {
        throw new Error('useBuddhism must be used within an BuddhismFormProvider');
    }
    return context;
};
