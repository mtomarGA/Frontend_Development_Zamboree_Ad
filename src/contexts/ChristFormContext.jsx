...
// --- Create Context ---
const ChristFormContext = createContext();

// --- Create Provider Component ---
export const ChristFormProvider = ({ children, onSuccess }) => {
    ...
    const handleLoadData = async (data) => {
        ...
        setGoogleMapData(
            ...
            // set only username extract last / after in the url
        setSocialMediaData({
    ...
    const handleSubmit = async () => {
        ...
        if (isEditMode) {
            const ress = await ChristService.update(editTempleId, body);
            console.log('Response:', ress);
        } else {
            const ress = await ChristService.create(body);
            console.log('Response:', ress);
        }
        setTempleTabManager({
            overview: true,
        ...
        router.push("/en/apps/spritual/christ/manage")
    }
    console.log("google map data", googleMapData);
    ...
    return (
        <ChristFormContext.Provider value={value}>
            {children}
        </ChristFormContext.Provider>
    );
};

// --- Custom Hook for Consuming Context ---
export const useChrist = () => {
    const context = useContext(ChristFormContext);
    if (context === undefined) {
        throw new Error('useChrist must be used within an ChristFormProvider');
    }
    return context;
};
