...
// --- Create Context ---
const JainismFormContext = createContext();

// --- Create Provider Component ---
export const JainismFormProvider = ({ children, onSuccess }) => {
    ...
    const handleSubmit = async () => {
        ...
        if (isEditMode) {
            const ress = await JainService.update(editTempleId, body);
            console.log('Response:', ress);
        } else {
            const ress = await JainService.create(body);
            console.log('Response:', ress);
        }
        setTempleTabManager({
            overview: true,
        ...
        router.push("/en/apps/spritual/jain/manage")
    }
    ...
    return (
        <JainismFormContext.Provider value={value}>
            {children}
        </JainismFormContext.Provider>
    );
};

// --- Custom Hook for Consuming Context ---
export const useJainism = () => {
    const context = useContext(JainismFormContext);
    if (context === undefined) {
        throw new Error('useJainism must be used within an JainismFormProvider');
    }
    return context;
};
